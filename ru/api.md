...
- #### `template`
  ...
  - `context.styles`: (string) любой встроенный CSS, который должен быть вставлена в заголовчный тег страницы. Обратите внимание, что это свойство будет автоматически заполнено при использовании `vue-loader` + `vue-style-loader` для CSS компонента.
...
- #### `shouldPreload`
  ...
  Функция, для управления какие файлы должны иметь `<link rel="preload">` в генерируемых ресурсах.
  ...
  Для других типов ресурсов, таких как изображений или шрифтов, предзагрузка может привести к ненужному увеличению передаваемой информации и даже к ухудшению производительности, поэтому предзагрузка зависит от реализуемого сценария. Вы можете точно контролировать, что требует предзагрузки, используя опцию `shouldPreload`:
...
- #### `runInNewContext`
  ...
  По умолчанию, для каждого рендеринга рендерер сборки будет создавать новый контекст V8 и повторно перевыполнять всю сборку. Это имеет некоторые преимущества — например, нам не нужно будет беспокоиться о проблеме «синглетона с хранимым состоянием», о которой упоминали ранее. Однако этот режим требует значительных затрат производительности, поскольку повторное выполнение сборки обходится дорого, особенно когда приложение становится большим.
...
- #### `basedir`
  ...
  Явное объявление базового каталога для серверной сборки для разрешения зависимостей из `node_modules` в нём. Это необходимо только в том случае, если сгенерированный файл сборки располагается в другом месте, в отличие от его используемых NPM-зависимостей, или когда ваш `vue-server-renderer` подключен npm-ссылкой в вашем текущем проекте.

- #### `cache`

  Реализация [кэширования на уровне компонентов](./caching.md#component-level-caching). Объект кэша должен реализовать следующий интерфейс (с использованием Flow нотаций):
  ...
  Обычное использование заключается в передаче [lru-cache](https://github.com/isaacs/node-lru-cache):
